...
export class MenuScene extends Phaser.Scene {
  ...
  private showDailyRewardModal(w: number, h: number, s: number) {
    ...
    const panelW = Math.min(360 * s, w * 0.9);
    const panelH = 500 * s;
    const px = w / 2;
    const py = h / 2;
    const panely = py - panelH / 2;
    ...
    border.lineStyle(3 * s, 0xffd700, 0.6);
    border.strokeRoundedRect(px - panelW / 2, panely, panelW, panelH, 20 * s);
    ...
    this.add.text(px, panely + 52 * s, '\u{1F381}', {
      fontSize: `${Math.round(56 * s)}px`,
    }).setOrigin(0.5).setDepth(102);

    const titleText = state === 'claimed' ? 'Already Claimed!' : 'Daily Reward!';
    this.add.text(px, panely + 102 * s, titleText, {
      fontFamily: 'Georgia, serif', fontSize: `${Math.round(26 * s)}px`, color: '#fff8e7', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(102);

    const divY = panely + 130 * s;
    const divider = this.add.graphics().setDepth(102);
    divider.lineStyle(1 * s, 0x8b6914, 0.4);
    divider.lineBetween(px - panelW / 2 + 30 * s, divY, px + panelW / 2 - 30 * s, divY);

    this.add.text(px, panely + 154 * s, `Streak: Day ${streak}`, {
      fontFamily: 'Georgia, serif', fontSize: `${Math.round(17 * s)}px`, color: '#c4b5fd', fontStyle: 'italic',
    }).setOrigin(0.5).setDepth(102);
    ...
    if (state !== 'claimed') {
      ...
      rewardTextObj = this.add.text(px, panely + 194 * s, rewardStr, {
        fontFamily: 'Georgia, serif', fontSize: `${Math.round(36 * s)}px`, color: '#ffd700', fontStyle: 'bold',
      ...
      const claimBtnW = Math.min(220 * s, panelW - 50 * s);
      const claimBtnH = 52 * s;
      const claimBtnX = px - claimBtnW / 2;
      const claimBtnY = panely + 234 * s;
      ...
      claimBorderObj.lineStyle(2.5 * s, 0xffd700, 0.8);
      ...
      claimTextObj.on('pointerover', () => {
        ...
        claimBorderObj!.lineStyle(3.5 * s, 0xffd700, 1);
        claimBorderObj!.strokeRoundedRect(claimBtnX, claimBtnY, claimBtnW, claimBtnH, 14 * s);
      ...
      claimTextObj.on('pointerdown', () => {
        ...
        overlay.destroy(); panel.destroy(); border.destroy(); divider.destroy();
    ...
    const cancelBtnW = Math.min(200 * s, panelW * 0.55);
    const cancelBtnH = 50 * s;
    const cancelBtnX = px - cancelBtnW / 2;
    const cancelBtnY = state === 'claimed' ? panely + 190 * s : panely + 310 * s;

    const cancelPanel = this.add.image(px, cancelBtnY + cancelBtnH / 2, 'wood_panel').setDepth(102);
    cancelPanel.setDisplaySize(cancelBtnW, cancelBtnH);
    const cancelBorder = this.add.graphics().setDepth(102);
    cancelBorder.lineStyle(2.5 * s, 0x4a3a6a, 0.7);
    cancelBorder.strokeRoundedRect(cancelBtnX, cancelBtnY, cancelBtnW, cancelBtnH, 14 * s);
