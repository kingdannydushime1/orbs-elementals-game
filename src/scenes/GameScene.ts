...
export class GameScene extends Phaser.Scene {
  ...
  private async processMatches(matches: { row: number; col: number }[], fromColorBomb: boolean) {
    ...
    if (toDestroy.size >= 4) {
      this.score += 50 * mult;
      this.showScorePopup(this.camW / 2, this.offY + this.gridW / 2 - 30 * this.sf, 50 * mult, 0xfbbf24);
    }
    if (toDestroy.size >= 5) {
      this.score += 100 * mult;
      this.showScorePopup(this.camW / 2, this.offY + this.gridW / 2 - 50 * this.sf, 100 * mult, 0xff6b35);
    }
